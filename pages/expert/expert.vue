<template>
  <view class="viewport">
    ...
    <view style="margin: 0 30rpx; border-radius: 50rpx; overflow: hidden;">
      <u-swiper :list="list1" indicator indicatorMode="line" circular height="180"></u-swiper>
    </view>
    ...
    <view class="hot-categories">
      <!-- 热门选项 -->
      <view class="tabs">
        <text v-for="(item, index) in subTypes" :key="item.type" class="text" :class="{ active: index === activeIndex }"
          @tap="activeIndex = index">{{ item.title }}</text>
      </view>
      <!-- 专家列表 -->
      <scroll-view v-for="(item, index) in subTypes" :key="item.type" v-show="activeIndex === index" scroll-y
        class="scroll-view-y">
  ...
  export default {
    data() {
      return {
        ...
        // 轮播图图片
        list1: [
          require('@/static/images/expert轮播1.png'),
          require('@/static/images/expert轮播2.png'),
          require('@/static/images/expert轮播3.png'),
        ],
        // 专家推荐数据
        expertsRecommend: [{
          id: 1,
          ...
          id: 2,
          name: "张教授",
          img:"/static/images/expert2.png"
        }, {
          id: 3,
          name: "李教授",
          img:"/static/images/expert3.png"
        }],
        // 热门问题分类
        subTypes: [{
            type: '1',
    ...
    methods: {
      ...
      handleNavigate({
        ...
        const query = Object.entries(extraParams).map(([key, value]) =>
          `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join('&') // 构造查询参数字符串
        const url = `${pagePath}?${query}` // 拼接带有额外参数的 URL
        uni.navigateTo({
          url
        }) // 使用uniapp的导航 API 进行页面跳转
      },
      // 获取专家推荐数据
